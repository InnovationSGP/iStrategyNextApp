import {errorHandler, expiryCalculator, validatePasswordWithRegExp} from "@/utils/common";
...
export default async function authRouter(
    ...
    if (req.method === "POST") {
        try {
            ...
            const validatePassword = validatePasswordWithRegExp(password)
            if (!validatePassword) return errorHandler("Password should have at least 8 characters with  at least a" +
                " symbol, upper and lower case letters and a number", res, 401);
